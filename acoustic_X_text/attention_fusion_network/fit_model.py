...
while(current_epoch_number < no_of_epochs):
	...
	hist = model.fit([training_COVAREP_X_FORMANT, training_transcript, training_X_gender], training_Y, batch_size = batch_size, epochs = 1)
	mse_train = hist.history['loss'][-1]
	mse_dev, mae_dev = model.evaluate([dev_COVAREP_X_FORMANT, dev_transcript, dev_X_gender], dev_Y, batch_size = dev_COVAREP_X_FORMANT.shape[0])
