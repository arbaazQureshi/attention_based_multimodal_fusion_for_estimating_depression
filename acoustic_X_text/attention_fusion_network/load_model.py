...
def load_model(location=None):
	...
	B = Concatenate(axis = 1)([COVAREP_X_FORMANT_shortened, transcript_elongated])
	...
	F = Lambda(lambda x : alpha[:, 0:1]*COVAREP_X_FORMANT_shortened + alpha[:, 1:2]*transcript_elongated)(alpha)
	...
	model = Model(inputs = [COVAREP_X_FORMANT, transcript, X_gender], outputs = Y)
